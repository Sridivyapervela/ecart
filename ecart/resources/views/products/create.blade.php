        ...
        <div class="row justify-content-center">
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form autocomplete="off" action="/products" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="form-group">
                                <label for="name">Product Name</label>
                                <input type="text" class="form-control {{ $errors->has('name') ? 'border-danger' : ''}}" id="name" name="name" value="{{old('name')}}" required autocomplete="name" >
                            ...
                            <input id="status"  placeholder="active/inactive" class="form-control @error('status') is-invalid @enderror" name="status" value="{{ old('status') }}" required autocomplete="status">
                            <small class="form-text text-danger">{!! $errors->first('status') !!}</small>
                            </div>

                            <div class="form-group">
                            <label for="category_id">{{ __('Category_id ') }}</label>
                            <input id="category_id" placeholder="select existing category_id" type="number" class="form-control @error('category_id') is-invalid @enderror" name="category_id" value="{{ old('category_id') }}" required autocomplete="category_id">
                        ...
                        <a class="btn btn-primary float-right" href="/products"><i class="fas fa-arrow-circle-up"></i> Back</a>
                    </div>
                </div>
            </div>
        </div>
